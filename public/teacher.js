(()=>{
  ...
  function groups(){
    return `<section class="panel"><div class="section-head"><div><p class="eyebrow">GRUPOS</p><h2>Central de grupos</h2></div><button class="btn" data-action="create-group">Novo grupo</button></div><p class="muted">Os integrantes podem cadastrar o texto, o link do formulário do próprio grupo e frases em uma lista compartilhada entre os grupos.</p><div class="bulk-bar"><strong><span id="group-selected-count">0</span> selecionado(s)</strong><select id="bulk-group-action"><option value="status:not_started">Marcar não iniciados</option><option value="status:in_progress">Marcar em andamento</option><option value="status:review">Enviar para revisão</option><option value="status:completed">Finalizar grupos</option><option value="lock">Bloquear receitas</option><option value="unlock">Liberar receitas</option></select><button class="btn" data-action="bulk-groups" disabled>Aplicar</button></div><div class="admin-group-grid">${data.groups.map(groupCard).join('')}</div></section>`;
  }
