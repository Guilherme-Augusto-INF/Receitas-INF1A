(()=>{
  ...
  const groupHref=group=>`../grupo/?slug=${encodeURIComponent(group.slug)}`;
  ...
  function groups(){
    return `<section class="panel"><div class="section-head"><div><p class="eyebrow">GRUPOS</p><h2>Central de grupos</h2></div><button class="btn" data-action="create-group">Novo grupo</button></div><p class="muted">Cada grupo pode ter um formulário obrigatório sobre seu tema para ser respondido por todos os alunos da turma.</p><div class="bulk-bar"><strong><span id="group-selected-count">0</span> selecionado(s)</strong><select id="bulk-group-action"><option value="status:not_started">Marcar não iniciados</option><option value="status:in_progress">Marcar em andamento</option><option value="status:review">Enviar para revisão</option><option value="status:completed">Finalizar grupos</option><option value="lock">Bloquear receitas</option><option value="unlock">Liberar receitas</option></select><button class="btn" data-action="bulk-groups" disabled>Aplicar</button></div><div class="admin-group-grid">${data.groups.map(groupCard).join('')}</div></section>`;
  }
  ...
  async function handleClick(event){
    ...
    if(button.dataset.action==='refresh')return load();if(button.dataset.action==='export')return exportCsv();if(button.dataset.action==='create-group')return createGroup(button);if(button.dataset.action==='bulk-students')return bulkStudents(button);if(button.dataset.action==='bulk-groups')return bulkGroups(button);
    ...
    if(button.dataset.editGroup)return editGroup(button.dataset.editGroup);if(button.dataset.groupForm)return editGroupForm(button.dataset.groupForm);if(button.dataset.reviewAction)return reviewAction(button);if(button.dataset.reviews)return showReviews(button.dataset.reviews);if(button.dataset.comment)return commentRecipe(button.dataset.comment);
  ...
  async function createGroup(button){const theme=prompt('Tema do novo grupo (ex.: Fibras, Enzimas, Minerais):');if(theme===null||!theme.trim())return;const description=prompt('Descrição opcional do grupo:','');if(description===null)return;setBusy(button,true,'Criando…');try{const created=await rpc('create_classroom_group',{p_theme:theme.trim(),p_description:description.trim()||null});toast(`${created.name} criado.`);await load();render('groups')}catch(error){toast(BioUI.friendlyError(error),'error');setBusy(button,false)}}
  ...
  async function editGroupForm(id){const g=data.groups.find(x=>x.id===id);if(!g)return;const text=prompt('Texto que aparecerá para toda a turma antes do formulário. Deixe vazio junto com o link para remover:',g.form_text||'Responda ao formulário sobre este tema. A atividade é obrigatória para todos os alunos da turma.');if(text===null)return;const url=prompt('Link do formulário (Google Forms, Microsoft Forms etc.). Deixe vazio junto com o texto para remover:',g.form_url||'https://');if(url===null)return;try{await rpc('set_group_form',{p_group_id:id,p_form_text:text.trim(),p_form_url:url.trim()==='https://'?'':url.trim()});toast(url.trim()&&url.trim()!=='https://'?'Formulário do grupo atualizado.':'Formulário removido.');await load();render('groups')}catch(error){toast(BioUI.friendlyError(error),'error')}}
